...
pub fn init_db(config_path: PathBuf) {
    ...
    // create db.cipherlock according to config file
...
impl DbFile {
    ...
    pub fn init(mut db_location: PathBuf) {
        ...
        let mut cipherlock_folder: PathBuf = util::get_homedir();

        cipherlock_folder.push(DIR_NAME);

        util::create_dir(&cipherlock_folder);

        let mut conf_name: PathBuf = PathBuf::new();
        conf_name.push(CONF_NAME);
        conf_name.set_extension(CONF_FILE_EXT);

        let config = Config::new(conf_name, cipherlock_folder);

        let db_cipherlock = DbFile::new(PathBuf::from(DB_NAME), db_location);

        let display = db_cipherlock.name.display();

        // converting db_cipherlock variable to json encoded string.
        let as_json: String = match serde_json::to_string(&db_cipherlock) {
